...
impl CookieStore {
    ...
    fn key_ref(&self) -> &Key {
        self.key.get_or_init(generate_key)
    }

    fn sign_cookie(&self, cookie: Cookie<'static>) -> Cookie<'static> {
        let mut jar = CookieJar::new();
        jar.signed_mut(self.key_ref()).add(cookie);
        jar.get(&self.name)
            .cloned()
...
impl FlashStore for CookieStore {
    async fn load_flash(&self, req: &mut Request, _depot: &mut Depot) -> Option<Flash> {
        match req.cookies().signed(self.key_ref()).get(&self.name) {
            None => None,
...
fn generate_key() -> Key {
    let mut key = [0; 64];
    getrandom::fill(&mut key).unwrap_or_else(|err| {
        panic!("failed to generate flash cookie key: {err:?}");
    });
    Key::from(&key)
}
