    ...
    fn try_from(entry: CachedEntry) -> Result<Self, Self::Error> {
        ...
        let body = match entry.body {
            ...
            CachedBody::Chunks(chunks) => {
                InnerCachedBody::Chunks(chunks.into_iter().map(|b| b.into()).collect())
            }
        };
        Ok(Self {
            status,
...
impl TryFrom<InnerCachedEntry> for CachedEntry {
    ...
    fn try_from(entry: InnerCachedEntry) -> Result<Self, Self::Error> {
        ...
        let body = match entry.body {
            ...
            InnerCachedBody::Chunks(chunks) => {
                CachedBody::Chunks(chunks.into_iter().map(|b| b.into()).collect())
            }
        };
        Ok(Self {
            status,
